...
fun HomeView(navController: NavController, viewModel: WishViewModel){
    ...
    Scaffold(topBar = {AppBarView(title = "WishList",{
        ...
        FloatingActionButton(modifier = Modifier.padding(all = 20.dp), contentColor = Color.White, backgroundColor = Color.Black,onClick = {
            Toast.makeText(context, "Button Clicked!", Toast.LENGTH_LONG).show()
        ...
        LazyColumn(modifier = Modifier
            ...
            items(wishlist.value, key = {wish ->wish.id}){
                ...
                SwipeToDismiss(state = dismissState, background = {
                    val color by animateColorAsState(if (dismissState.dismissDirection == DismissDirection.EndToStart) Color.Red else Color.Transparent, label = "")
                    val alignment = Alignment.CenterEnd
                    Box(modifier = Modifier.fillMaxSize().background(color).padding(horizontal = 12.dp), contentAlignment = alignment){
